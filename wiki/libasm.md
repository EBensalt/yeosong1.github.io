...
| 명령어 | 수행 내용 | 요약(?) |
| --- | --- | --- |
| JZ | ZF 가 세트일 때만 분기 | if(ZF == 1) |
| JNZ | ZF 가 언세트일 때만 분기 | if(ZF == 0) |
| JO | OF 가 세트일 때만 분기 | if(OF == 1) |
| JNO | OF 가 언세트일 때만 분기 | if(OF == 0) |
| JS | SF 가 세트일 때만 분기 | if(SF == 1) |
| JNS | SF 가 언세트일 때만 분기 | if(SF == 0) |
| JC | CF 가 세트일 때만 분기 | if(CF == 1) |
| JNC | CF 가 언세트일 때만 분기 | if(CF == 0) |
| JP | PF 가 세트일 때만 분기 | if(PF == 1) |
| JNP | PF 가 언세트일 때만 분기 | if(PF == 0) |
