...
## ex01 배관 문제 -- 메모리 할당/해제 -- 스택으로든 힙으로든 하여간 누수 없도록 수정
...
- **Brain 클래스**를 만듭니다. 두뇌에 적합하다고 생각되는 모든 것을 사용하여.
  - 멤버 함수: **identify() 함수** 
    - 문자열을 16진수 포맷으로 반환하는 함수이다 -- this 혹은 현재 인스턴스의 주소를 반환
    - (문자열 = 메모리에 있는 brain의 주소)
    - (16진수 포맷 = 접두사 0x 예시: "0x194F87EA")
...
어떤 경우에 **Weapon** 포인터로 저장하는 것이 적절한가요? 레퍼런스로? 왜죠? <br>
요구사항에 비추어 볼 때 이것이 최선의 선택일까요? <br>
...
- **replace**라는 프로그램을 만듭니다.
  - **filename**과 두 개의 문자열(비어 있지 않은 **s1**과 **s2**)을 받는 프로그램이다.
  - 거기 나오는 모든 **s1**을 **s2**으로 바꿉니다.
  - 그 결과를 파일을 열고 **FILENAME.replace**에 씁니다.
- 물론 오류를 최대한 처리하고
- C의 파일 조작 함수는 사용하지 않습니다. 그건 치팅이고 치팅은 나쁘니까 그죠?
- 프로그램이 작동하는 것을 보여주기 위해 몇 가지 테스트 파일을 제출합니다.

-----------
여기서부터는 해도 점수와는 관계 없음

## ex08 화려한 스위치가 없다는 말은 들어본 적이 없다. -- if나 switch문 안쓰고 뭐 짜기
## ex09 오버 로깅 -- 로그 남기는 뭐 짜기
## ex10 꼬리 아홉개 달린 Cat -- cat 옵션 없이 구현하기.  
