...
이 camera-to-world-matrix를 점 O와 P에 적용하면 벡터 ||O'P'|| (여기서 O'는 점 O이고 P'는 camera-to-world-matrix에 의해 변환 된 점 P이다.) world 공간에서 광선의 정규화 된 방향을 나타냅니다 (그림 8). 카메라에서 world로 변환을 O 및 P에 적용하면 이 두 점이 카메라 공간에서 world 공간으로 변환됩니다. 다른 옵션은 카메라가 기본 위치(벡터 OP)에 있는 동안 광선 방향을 계산하고, 이 벡터에 camera-to-world-matrix를 적용하는 것입니다.

카메라 좌표계가 카메라와 어떻게 움직이는지 확인하세요. psuedo 코드는 카메라 변형을 설명하기 위해 쉽게 수정할 수 있습니다(회전 및 변환, 카메라 배율 조정은 특히 권장되지 않음).
