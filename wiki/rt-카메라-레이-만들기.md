...
또한 이미지가 정사각형이므로 이미지가 투영되는 이미지 평면의 일부도 반드시 정사각형입니다. 우리가 곧 설명 할 이유 때문에,이 투영 영역의 크기는 2 x 2 단위입니다 (그림 2). 또한 래스터 이미지가 픽셀로 구성되어 있음을 알고 있습니다. 우리가 필요로하는 것은 래스터 공간에서 이들 픽셀의 좌표와 동일한 픽셀의 좌표 사이에서 월드 공간으로 표현되는 관계를 찾는 것입니다. 이 프로세스에는 그림 5에 표시된 몇 가지 단계가 필요합니다.
