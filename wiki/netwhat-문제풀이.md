...
## 주소 지정 방식
- [클래스 단위 주소 지정(Classful)](#IP-주소-클래스란-무엇입니까?)
- [서브넷 사용(Subnetted)](#서브넷,-서브넷마스크)
- [클래스 비사용 주소 지정(Classless)](#CIDR:-클래스-비사용-도메인-간-라우팅) <- *현재 표준* 
...
## 서브넷, 서브넷 마스크
마스크를 사용하면 라우터가 32 비트 주소 전체를 처리해야하는 부담을 줄여주며,
단순히 마스크에 의해 선택된 비트들만 보도록 해준다.
...
## 네트워크 계층이란 무엇입니까?
네트워크 주소부여(IP),경로설정(Route) 하는 곳. 계층적(hierarchical) 구조.
