...
## 4. 레이, 간단한 카메라, 배경
### 4.1 ray class
모든 광선 추적기에 있는 한 가지는 광선 클래스와 광선을 따라 보이는 색상에 대한 계산입니다. 광선을 함수 𝐏 (𝑡) = 𝐀 + 𝑡𝐛로 생각해 봅시다. 여기 𝐏는 3D에서 선을 따라 3D 위치입니다. 𝐀은 광선 원점이고 𝐛은 광선 방향입니다. 광선 매개 변수 𝑡은 실수입니다 (코드에서 두 배). 다른 𝑡을 연결하면 𝐏 (𝑡)이 광선을 따라 점을 이동합니다. 음수 𝑡 값을 더하면 3D 라인의 아무 곳이나 갈 수 있습니다. 양수 𝑡의 경우 𝐀 앞에 있는 부분만 얻습니다. 이것은 종종 반직선 또는 레이라고 하는 것입니다.
