...
저는 수년 동안 많은 그래픽 수업을 가르쳤습니다. 종종 수업들을 ray tracing으로 하는데, 모든 코드를 (직접) 작성해야하지만 API 없이도 멋진 이미지를 얻을 수 있기 때문입니다. 저는 당신이 가능한 한 빨리 멋진 프로그램을 만들 수 있도록 내 코스 노트를 방법론으로 바꾸기로 결정했습니다. 완전한 기능을 갖춘 ray tracer는 아니지만, 영화에 ray tracing을 필수 요소로 만드는 간접적인 조명을 갖고 있습니다. 다음 단계를 따르십시오. 흥미를 느끼고 원한다면, 당신이 생산하는 ray tracer의 아키텍처는 더 광범위한 ray tracer로 확장하는 데 좋을 것 입니다.
...
그 빛들은 또한 반사되기보다는 흡수될 것입니다.<br>
표면이 어두울수록 흡수 가능성이 높아집니다.(그게 그래서 어두운거에요!)<br>
실제로, 방향을 무작위로 만드는 모든 알고리즘이 매트해보이는 표면을 생성합니다.<br>
이를 수행하는 가장 간단한 방법 중 하나는, 이상적인 확산 표면에 정확히 맞는 것으로 밝혀졌습니다.<br>
(저는 수학적으로 이상적인 Lambertian에 가까운, 게으른 해킹을 사용했습니다.)
(*[Lambertian reflectance](https://ko.wikipedia.org/wiki/%EB%9E%8C%EB%B2%A0%EB%A5%B4%ED%8A%B8_%EB%B0%98%EC%82%AC): 크래픽스에서 난반사가 이루어지는 표면을 묘사하기 위해 사용하는 모델)

(독자 Vassillen Chizhov는 게으른 해킹이 실제로 게으른 해킹이며 부정확하다는 것을 증명했습니다. 이상적인 Lambertian의 올바른 표현은 그다지 더 많은 작업이 아니며, 장의 맨끝에 제시됩니다.)

표면의 적중점(hit point) 𝑝에 접하는 두 개의 단위 반지름 구가 있습니다.<br>
