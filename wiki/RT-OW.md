...
표면의 히트 포인트 𝑝에 접하는 두 개의 단위 반경 구가 있습니다. 이 두 구체는 중심이 (𝐏 + 𝐧) 및 (𝐏−𝐧)이며, 여기서 𝐧은 표면의 법선입니다. 중심이 (𝐏−𝐧) 인 구는 표면 내부로 간주되는 반면 중심 (𝐏 + 𝐧)이있는 구는 표면 외부로 간주됩니다. 광선 원점과 같은 표면에있는 접선 단위 반지름 구를 선택합니다. 이 단위 반경 구 내부의 임의의 지점 𝐒을 선택하고 히트 지점 𝐏에서 임의의 지점 𝐒까지 광선을 보냅니다 (이것은 벡터 (𝐒−𝐏)).
